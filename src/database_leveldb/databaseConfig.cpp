...
DatabaseConfig::DatabaseConfig( const char* source)
	:m_cachesize_kb(0),m_compression(true)
{
	...
	for (; ci != ce; ++ci)
	{
		...
		{
			if (compression_defined) throw std::runtime_error( "duplicate definition of 'compression' in the levelDB key value store database config");
			m_compression = yesNo( "compression", ci->second);
			compression_defined = true;
		}
		else if (ci->first == "cache")
		{
			if (cache_defined) throw std::runtime_error( "duplicate definition of 'cache' in the levelDB key value store database config");
	...
	if (m_path.empty())
	{
		throw std::runtime_error( "no path defined in config for the levelDB key value store database");
	}
}
