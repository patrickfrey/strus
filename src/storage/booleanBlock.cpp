...
bool BooleanBlock::Node::tryExpandRange( const Index& to_)
{
	...
	{
		case DiffNode:
			if (elemno >= to_) throw strus::runtime_error( _TXT( "internal: call try expand range with node end within node"));
			alt.diff += (to_ - elemno);
			elemno = to_;
			return true;
...
void BooleanBlock::defineRange( const Index& elemno, const Index& rangesize)
{
	...
	if (nodearsize)
	{
		...
		if (elemno < from_)
		{
			throw strus::runtime_error( _TXT( "internal: ranges not appended in order in boolean block"));
		}
		if (elemno <= to_)
		{
			// [from_ <= elemno <= to_]
