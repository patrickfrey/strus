...
StorageMetaDataTransaction::StorageMetaDataTransaction(
	...
	,m_nofOperations(0),m_commit(false),m_rollback(false)
...
bool StorageMetaDataTransaction::commit()
{
	...
	CATCH_ERROR_MAP_RETURN( _TXT("error renaming element: %s"), *m_errorhnd, false);
}
		...
		MetaDataElement::Type type)
{
	...
	CATCH_ERROR_MAP( _TXT("error changing element type: %s"), *m_errorhnd);
}
